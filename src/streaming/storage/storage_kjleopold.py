...
from streaming.data_validation.data_validation_kjleopold import add_validation_errors
...
def log_storage_summary(db_path: Path) -> None:
    ...
    with duckdb.connect(str(db_path)) as conn:
        ...
        customer_rows = conn.execute(sql_by_customer_value).fetchall()
    ...
    LOG.info("DuckDB sales by customer value:")
    for customer_value, transaction_count in customer_rows:
        LOG.info(f"  {customer_value}: {transaction_count} transaction(s)")
